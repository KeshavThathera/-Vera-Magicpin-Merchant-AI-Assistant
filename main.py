...
async def startup_scheduler():
    ...
    scheduler.add_job(scheduled_keep_alive, "interval", minutes=10)
    scheduler.start()
    logger.info("Keep-alive scheduler started - will ping every 10 minutes via loopback")
...
async def tick(body: TickBody):
    ...
    # ── 2. COMPOSE ACTIONS ───────────────────────────────────────────────────
    for trg_score, trg_id, trg, merchant_id, merchant, category_slug, category in candidates:
        # Time guard — stop composing if we're close to deadline
        ...
        # Per-trigger budget: fair share of remaining time, capped at 5s
        # Per-trigger budget: fair share of remaining time, capped at 7s
        rem = deadline - time.time()
        # Assume we only need to finish ~5 more if we're halfway through
        trg_budget = min(7.0, rem / max(2.0, min(10, len(candidates) - len(actions))))

        # Compose — fallback fires automatically if budget is too thin
        if trg_budget < 1.5:
            composed = build_grounded_fallback(category, merchant, trg, customer)
        else:
            try:
                composed = await asyncio.wait_for(
                    safe_compose(category, merchant, trg, customer, turn=1, budget=trg_budget),
                    timeout=trg_budget + 2.0
                )
            except Exception as e:
